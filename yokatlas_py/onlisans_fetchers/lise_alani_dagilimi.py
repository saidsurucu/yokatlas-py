...
async def fetch_lise_alani_dagilimi(program_id, year):
    if year not in [2021, 2022, 2023]:
        return {"error": "Invalid year. Only 2021, 2022, and 2023 are supported."}

    base_url = "https://yokatlas.yok.gov.tr"
    url_suffix = f"/{year}/content/onlisans-dynamic/3050b.php?y={program_id}" if year != 2023 else f"/content/onlisans-dynamic/3050b.php?y={program_id}"
    ...
    for row in rows:
        ...
        for i, header in enumerate(headers):
            if i == 0:  # Lise Alanı
                row_data['Lise Alanı'] = cols[i].get_text(strip=True)
            elif i < len(cols):
        ...
        if row_data['Lise Alanı'] == 'Toplam':
            toplam = row_data
        else:
            result.append(row_data)

    return {
        'lise_alani_dagilimi': result,
        'toplam': toplam
    }
