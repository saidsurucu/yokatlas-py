...
async def fetch_degisim_programi_bilgileri(
    ...
    url_suffix = (
        f"/{year}/content/onlisans-dynamic/2040.php?y={program_id}"
        if year != 2024
        else f"/content/onlisans-dynamic/2040.php?y={program_id}"
    )
    ...
    if table:
        ...
        result = {"has_data": True, "degisim_programlari": data}
    else:
        # If the table does not exist, look for the info message
        info_div = soup.find(
            "div", string=lambda text: "Değişim Programı ile" in text if text else False
        )
        ...
        if info_div:
            ...
            result = {"message": message, "giden_gelen": giden_gelen, "has_data": False}
        else:
            result = {
